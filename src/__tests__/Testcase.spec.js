...
it('should produce a <failure>', () => {
  ...
  const result = new Testcase(testcase);
  const report = xml(result);
  const expected = `<test-case name="boo/foo should foo bar" executed="True" result="Failure" time="0"><failure><message>Assertion error</message><stack-trace>Assertion error</stack-trace></failure></test-case>`;
  expect(report).toEqual(expected);
});
